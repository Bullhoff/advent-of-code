...
class Part1 {
	...
	iterate() {
		...
		for (let i = 0; i < input_data.length; i++) {
			for (let y = 0; y < input_data[i].length; y++) {
				if (i == input_data.length - 1 || i == 0 || y == input_data[i].length - 1 || y == 0 || this.checkIfVisible(matrix[i], y) || this.checkIfVisible(invertedMatrix[y], i)) {
					this.visibleTrees += 1;
	...
	checkIfVisible(row, pos) {
		...
		for (let i = 0; i < row.length; i++) {
			if (i < pos && Number(row[i]) >= Number(row[pos])) visible.left = false;
			else if (i > pos && Number(row[i]) >= Number(row[pos])) visible.right = false;
		}
		let vis = false;
		if (visible.left == true || visible.right == true) vis = true;
		return vis;
	}
}
...
class Part2 {
	...
	iterate() {
		...
		for (let i = 0; i < matrix.length; i++) {
			for (let y = 0; y < matrix[i].length; y++) {
				...
				if (!this.scores[i]) this.scores.push([]);
				if (!this.scores[i][y]) this.scores[i].push([]);
	...
	getScore(trees, treeHouse) {
		for (let i = 0; i < trees.length; i++) {
			if (trees[i] >= treeHouse || i == trees.length - 1) return i + 1;
		}
		return 0;
	}
	printMatrix() {
		...
		for (let i = 0; i < arr.length; i++) {
			for (let y = 0; y < arr[i].length; y++) {
				...
				if (arr[i][y].toString().length > 1) fullString += `\x1B[4m`;
...
function invertMatrix(matrix) {
	...
	for (let i = 0; i < matrix.length; i++) {
		for (let y = 0; y < matrix[i].length; y++) {
			if (!invertedMatrix[y]) invertedMatrix.push([]);
...
console.log('\r\n p1:');
console.log(p1.printMatrix);
console.log('\r\n p2:');
console.log(p2.printMatrix());
