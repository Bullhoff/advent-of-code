...
function getCurrentdirObj(currentDirArr, dir, rowInput, i = -1, tt = '') {
	i = i + 1;
	if (!dir.hasOwnProperty(currentDirArr[i])) dir[currentDirArr[i]] = {};

	if (i == currentDirArr.length - 1 && rowInput[0] != 'dir') dir[currentDirArr[i]][rowInput[1]] = rowInput[0];
	else if (i < currentDirArr.length - 1) dir[currentDirArr[i]] = getCurrentdirObj(currentDirArr, dir[currentDirArr[i]], rowInput, i);

	return dir;
}

function getDirectoriesSize(dir, folderId = '/', summary = {}) {
	if (!summary.hasOwnProperty(folderId)) summary[folderId] = Number(0);

	for (const [key, value] of Object.entries(dir)) {
		if (typeof value == 'string') summary[folderId] = summary[folderId] + Number(value);
		else summary[folderId] = summary[folderId] + getDirectoriesSize(value, folderId + key + '/', summary)[folderId + key + '/'];
	}
	return summary;
}
...
let p2 = Object.values(p1.dirSummary)
	.filter((val) => val > 30000000 - (70000000 - p1.dirSummary['/']))
	.sort((a, b) => a - b)[0]; // 10475598
