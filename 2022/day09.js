...
class Part1 {
	...
	iterate() {
		...
		for (let i = 0; i < moves.length; i++) {
			hLocation = [Number(hLocation[0]) + Number(moves[i][0]), Number(hLocation[1]) + Number(moves[i][1])];
			let diff = [Math.abs(hLocation[0] - tLocation[0]), Math.abs(hLocation[1] - tLocation[1])];
	...
	addPositionVisit(plane, tLocation) {
		...
		if (plane[tLocation[0]][tLocation[1]] == 0) this.positionsVisited += 1;
	...
	setStuff(hMoves) {
		...
		for (let i = 0; i < moves.length; i++) {
			...
			if (currentPos[0] > largestPos[0]) largestPos[0] = currentPos[0];
			if (currentPos[1] > largestPos[1]) largestPos[1] = currentPos[1];
			//if(moves[i][0] > 0)
	...
	addLayer(plane, currentLocation, steps) {
		for (let i = 0; i < steps; i++) {
			...
			if (!plane[currentLocation[0] + i]) plane.push(0);
		}
		return plane;
	}
	...
	translateMove(move) {
		...
		if (direction == 'R') return [steps, 0];
		else if (direction == 'L') return [-steps, 0];
		else if (direction == 'U') return [0, steps];
		else if (direction == 'D') return [0, -steps];
	}
}
...
class Part2 {
	...
	getMaxCoordinates() {
		...
		for (let i = 0; i < this.moves.length; i++) {
			...
			if (currentPos.x > max.r) max.r = currentPos.x;
			if (currentPos.x < max.l) max.l = currentPos.x;
			if (currentPos.y > max.u) max.u = currentPos.y;
			if (currentPos.y < max.d) max.d = currentPos.y;
		}
		return max;
	}
	createPlane() {
	...
	translateMoves(movesRaw) {
		...
		for (let i = 0; i < movesRaw.length; i++) {
			...
			if (direction == 'R') moves.push({ x: steps, y: 0 });
			else if (direction == 'L') moves.push({ x: -steps, y: 0 });
			else if (direction == 'D') moves.push({ x: 0, y: -steps });
			else if (direction == 'U') moves.push({ x: 0, y: steps });
		}
		return moves;
	}
}
